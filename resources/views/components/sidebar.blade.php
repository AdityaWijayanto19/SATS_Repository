<aside class="w-60 flex-shrink-0 flex flex-col py-5" style="background: rgb(0,83,63);">
    <nav class="flex flex-col gap-0.5">

        <!-- Dashboard -->
        <a href="{{ route('dashboard') }}" class="flex items-center gap-2 px-5 py-2.5 text-md font-medium cursor-pointer transition-all duration-150 border-l-2 rounded-r-lg
                  {{ request()->routeIs('dashboard') ? 'text-white border-[#7de0c0] bg-white/10' : 'text-white/60 border-transparent hover:text-white hover:border-white/30 hover:bg-white/[0.06]' }}">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                <path d="M3 13h8V3H3v10zm0 8h8v-6H3v6zm10 0h8V11h-8v10zm0-18v6h8V3h-8z"/>
            </svg>
            Dashboard
        </a>

        <!-- Input Data Pasien -->
        <a href="{{ route('input-data-pasien') }}" class="flex items-center gap-2 px-5 py-2.5 text-md font-medium cursor-pointer transition-all duration-150 border-l-2 rounded-r-lg
                  {{ request()->routeIs('input-data-pasien') ? 'text-white border-[#7de0c0] bg-white/10' : 'text-white/60 border-transparent hover:text-white hover:border-white/30 hover:bg-white/[0.06]' }}">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V5a2 2 0 0 0-2-2zm-7 3a1 1 0 0 1 1 1v3h3a1 1 0 0 1 0 2h-3v3a1 1 0 0 1-2 0v-3H8a1 1 0 0 1 0-2h3V7a1 1 0 0 1 1-1z"/>
            </svg>
            Input Data Pasien
        </a>

        <!-- Laporan -->
        <a href="{{ route('laporan') }}" class="flex items-center gap-2 px-5 py-2.5 text-md font-medium cursor-pointer transition-all duration-150 border-l-2 rounded-r-lg
                  {{ request()->routeIs('laporan') ? 'text-white border-[#7de0c0] bg-white/10' : 'text-white/60 border-transparent hover:text-white hover:border-white/30 hover:bg-white/[0.06]' }}">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6zm-1 7V3.5L18.5 9H13zM8 13h8v1.5H8V13zm0 3h8v1.5H8V16zm0-6h3v1.5H8V10z"/>
            </svg>
            Laporan
        </a>

        <!-- <div class="mx-5 my-3" style="border-top: 0.5px solid rgba(255,255,255,0.12);"></div>

        <a href="{{ route('dashboard') }} class="flex items-center gap-2 px-5 py-2.5 text-md font-medium cursor-pointer transition-all duration-150 border-l-2 rounded-r-lg
                  {{ request()->routeIs('settings') ? 'text-white border-[#7de0c0] bg-white/10' : 'text-white/60 border-transparent hover:text-white hover:border-white/30 hover:bg-white/[0.06]' }}">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                <path d="M19.14 12.94c.04-.3.06-.61.06-.94s-.02-.64-.07-.94l2.03-1.58a.49.49 0 0 0 .12-.61l-1.92-3.32a.49.49 0 0 0-.59-.22l-2.39.96a6.97 6.97 0 0 0-1.62-.94l-.36-2.54A.484.484 0 0 0 14 2h-4a.484.484 0 0 0-.48.41l-.36 2.54a7.27 7.27 0 0 0-1.62.94l-2.39-.96a.48.48 0 0 0-.59.22L2.74 8.87a.48.48 0 0 0 .12.61l2.03 1.58c-.05.3-.07.62-.07.94s.02.64.07.94l-2.03 1.58a.49.49 0 0 0-.12.61l1.92 3.32c.12.22.37.29.59.22l2.39-.96c.5.36 1.04.67 1.62.94l.36 2.54c.05.28.3.48.48.48h4c.28 0 .46-.2.48-.41l.36-2.54a7.27 7.27 0 0 0 1.62-.94l2.39.96c.22.08.47 0 .59-.22l1.92-3.32a.49.49 0 0 0-.12-.61l-2.01-1.58zM12 15.6a3.6 3.6 0 1 1 0-7.2 3.6 3.6 0 0 1 0 7.2z"/>
            </svg>
            Settings
        </a> -->

    </nav>
</aside>